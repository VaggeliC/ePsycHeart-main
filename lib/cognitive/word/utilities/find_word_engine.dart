...
import 'package:epsycheart/cognitive/word/utilities/find_word_check_orientations.dart';
...
import 'package:epsycheart/cognitive/word/utilities/find_word_skip_orientations.dart';
import 'package:epsycheart/cognitive/word/utilities/find_word_solved.dart';
import 'package:epsycheart/cognitive/word/utilities/find_word_orientations.dart';

class FindWordEngine {
  /// The definition of the orientation, calculates the next square given a
  /// starting square (x,y) and distance (i) from that square.
  final Map<FindWordOrientation, FindWordOrientationFn>
      orientations; // = findWordOrientations;
  ...
  final Map<FindWordOrientation, FindWordCheckOrientationFn>
      _checkOrientations; // = findWordCheckOrientations;
  ...
  final Map<FindWordOrientation, FindWordOrientationFn>
      _skipOrientations; //=      findWordSkipOrientations;
  ...
  bool _placeWordInPuzzle(
    ...
    final List<FindWordLocation> locations =
    ...
    final FindWordLocation sel = locations[Random().nextInt(locations.length)];
    _placeWord(
      puzzle,
      word,
      sel.x,
  ...
  List<FindWordLocation> _findBestLocations(
    /// The current state of the puzzle
    List<List<String>> puzzle,
    ...
    List<FindWordLocation> locations = [];
    ...
    for (var i = 0; i < options.orientations.length; i++) {
      final FindWordOrientation orientation = options.orientations[i];
      final FindWordOrientationFn next = orientations[orientation]!;
      final FindWordCheckOrientationFn check = _checkOrientations[orientation]!;
      final FindWordOrientationFn skip = _skipOrientations[orientation]!;
      int x = 0;
      int y = 0;
      ...
      while (y < height) {
        ...
        if (check(x, y, height, width, wordLength)) {
          ...
          FindWordPosition nextPossible = skip(x, y, wordLength);
          x = nextPossible.x;
          y = nextPossible.y;
        }
      }
  ...
  int _calcOverlap(
    ...
    FindWordOrientationFn fnGetSquare,
  ) {
    int overlap = 0;

    // traverse the squares to determine if the word fits
    for (var i = 0; i < word.length; i++) {
      final FindWordPosition next = fnGetSquare(x, y, i);
      String? square;
      try {
  ...
  List<FindWordLocation> _pruneLocations(
  ...
  FindWordNewPuzzle newPuzzle(
    ...
    final List<String> wordList = [...words]..sort((String a, String b) {
        return b.length - a.length;
      });
    // create new options instance of the settings
    final FindWordSettings options = FindWordSettings(
    ...
    if (options.fillBlanks != null) {
      ...
      if (options.fillBlanks is Function) {
        ...
        extraLetterGenerator = () {
          return defaultLetters[Random().nextInt(defaultLetters.length)];
        };
      }
  ...
  /// final WordSearch wordSearch = WordSearch();
  /// final FindWordNewPuzzle newPuzzle = wordSearch.newPuzzle(wl, ws);
  /// final FindWordSolved solved = wordSearch.solvePuzzle(newPuzzle.puzzle, ['dart']);
  /// // Outputs found and not found words
  /// ```
  ///
  FindWordSolved solvePuzzle(
    // The current state of the puzzle
    List<List<String>> puzzle,
    ...
    FindWordSettings options = FindWordSettings(
      height: puzzle.length,
      width: puzzle[0].length,
    );
    // Create new instance of the solved interface
    FindWordSolved output = FindWordSolved();
